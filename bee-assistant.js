(() => {
    ...
    function getOccasionTheme() {
        const forced = (document.body?.dataset?.occasion || '').toLowerCase().trim();
        if (forced) return forced;

        const today = new Date();
        const month = today.getMonth();
        const day = today.getDate();
        const isValentineWindow = month === 1 && day >= 7 && day <= 21;
        return isValentineWindow ? 'valentine' : 'default';
    }

    const OCCASION_THEME = getOccasionTheme();
    ...
    function buildAnnouncements() {
        ...
        if (OCCASION_THEME === 'valentine') {
            list.unshift({ text: 'Happy Valentine season! Sending love from Team Dhanu Tech 💖', href: WA_LINK, cta: 'Share your requirement', external: true });
        }
        return list;
    }

    function createLoveRainLayer() {
        const node = document.createElement('div');
        node.className = 'honeybee-love-rain';
        node.setAttribute('aria-hidden', 'true');
        return node;
    }

    function emitLoveDrop() {
        if (!loveRainLayer || OCCASION_THEME !== 'valentine') return;
        const drop = document.createElement('span');
        drop.className = 'love-drop';
        drop.textContent = Math.random() < 0.8 ? '❤' : '💖';
        drop.style.left = `${Math.random() * 100}vw`;
        drop.style.animationDuration = `${4.6 + Math.random() * 3.2}s`;
        drop.style.fontSize = `${11 + Math.random() * 11}px`;
        drop.style.opacity = `${0.35 + Math.random() * 0.5}`;
        loveRainLayer.appendChild(drop);
    ...
    function init() {
        ...
        controls.addEventListener('click', (e) => {
            ...
            if (role === 'hide') {
                ...
                if (loveRainLayer) loveRainLayer.classList.toggle('is-hidden', state.hidden);
        ...
        if (OCCASION_THEME === 'valentine') {
            for (let i = 0; i < 16; i += 1) {
                setTimeout(emitLoveDrop, i * 180);
            }
            setInterval(emitLoveDrop, LOW_END ? 640 : 340);
        }
